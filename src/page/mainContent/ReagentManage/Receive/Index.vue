<template>
	<div class="app-container Receive">
		<!-- 头部input搜索 -->
		<div class="Search_Top_Input">
			...
			<div class="addNew" style="width: 150px !important">
				<span @click="receive(selectList, 'batch')"> 批量领用</span>
			</div>
		</div>
		<!-- table -->
		<sys-table 
		...
		>
			<template slot-scope="scope" slot="operate">
				<el-button @click="receive(scope.row, 'one')" type="text" size="small">领用</el-button>
			</template>
		</sys-table>
		...
		getTableList() {
			...
			setTimeout(() => {
				this.tableHeader = [
					...
					{ columnValue: "data5", columnName: "功能" },
					...
					{ columnValue: "data9", columnName: "整件归还" },
				];
